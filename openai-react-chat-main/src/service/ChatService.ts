...
import {ChatCompletion, ChatCompletionMessage, ChatCompletionRequest, SendChatMessage, ChatMessage, ChatMessagePart, Role} from "../models/ChatCompletion";
...
export class ChatService {
  ...
  static async sendMessageExistingConverstaion(chatUserId: number, converstaionId: number, messages: ChatMessage[], callback: (content: string,fileDataRef: FileDataRef[]) => void): Promise<any> {
    ...
    const requestBody: SendChatMessage = {
      chat_user_id: chatUserId,
      chat_message_content: '',
      user_conversation_id: converstaionId,
    };
    ...
    try {
      ...
      response = await post<Response>('/sendChatMessage', JSON.stringify(requestBody));
    ...
    if (response.body) {
      ...
      try {
        debouncedCallback(JSON.stringify(response.body));
      } catch (error) {
        if (error instanceof Error && error.name === 'AbortError') {
